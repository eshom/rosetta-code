...
send_messages <- function(sockets) {
        ...
        with(socksub, send_named(sockets, paste0(time, nickname, ": ",
                                                 message)))
...
update_nicknames <- function(sockets) {
        ...
        is_taken <- with(sockets, (tolower(message) %in%
                                  tolower(sockets$nickname)) & !is.na(message))
        ...
        if (any(sent_ok)) {
                send_named(sockets, paste0(sockets$nickname[sent_ok],
                                           " has connected."))
        }
